def pbGetCoinString
  return _INTL("{1} FP", $player.coins)
end
...
class PokemonMartAdapter
  ...
  def getDisplayCoinPrice(item, selling = false)
    price = getPrice(item, selling).to_s_formatted
	if GameData::MapMetadata.get($game_map.map_id)&.has_flag?("Festival")
		return _INTL("{1} FP",price)
	else
...
class BuyAdapter
  ...
  def getDisplayCoinPrice(item)
    @adapter.getDisplayCoinPrice(item, false)
  end
...
class CoinMart_Scene
  ...
  def pbChooseNumber_Coin(helptext, item, maximum)
    curnumber = 1
    ret = 0
    ...
    using(numwindow = Window_AdvancedTextPokemon.new("")) do   # Showing number of items
      ...
      numwindow.text = _INTL("x{1}<r>{2} FP", curnumber, (curnumber * itemprice).to_s_formatted)
...
class CoinMartScreen
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if @adapter.getPoints < price
	    if $game_switches[125]
		  pbDisplayPaused(_INTL($game_variables[1]))
		else
      ...
      if added == quantity
		...
		if $game_switches[125]
		  pbDisplayPaused(_INTL($game_variables[3]))
		else
  ...
  def pbSellScreen
    ...
    loop do
      ...
      price /= 2
      price *= qty
      if pbConfirm(_INTL("I can pay {1} FP.\nWould that be OK?", price.to_s_formatted))
        old_money = @adapter.getPoints
        @adapter.setMoney(@adapter.getPoints + price)
        #$stats.money_earned_at_marts += @adapter.getPoints - old_money
        qty.times { @adapter.removeItem(item) }
        sold_item_name = (qty > 1) ? itemnameplural : itemname
        pbDisplayPaused(_INTL("You turned over the {1} and got {2} FP.",
...
def pbCoinMart(stock, speech = nil, cantsell = false)
  ...
  cmd = pbMessage(speech || _INTL("Please, take your time!"), commands, cmdQuit + 1)
  loop do
    if cmdBuy >= 0 && cmd == cmdBuy
      scene = CoinMart_Scene.new
