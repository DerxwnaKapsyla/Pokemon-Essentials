...
class BattlePointShopAdapter
  ...
  def getBPString
    return _INTL("{1} FP", $player.battle_points.to_s_formatted)
  end
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if @adapter.getBP < price
        pbDisplayPaused(_INTL("You don't have enough FP."))
        next
      end
      ...
      if @adapter.getBP < price
        pbDisplayPaused(_INTL("I'm sorry, you don't have enough FP."))
        next
      end
      added = 0
      quantity.times do
...
def pbBattlePointShop(stock, speech = nil)
  ...
  if speech.nil?
    ...
    pbMessage(_INTL("We can exchange your FP for fabulous items."))
  else
    pbMessage(speech)
  end
  ...
  pbMessage(_INTL("Please visit us again when you have saved up more FP."))
