...
def pbPokemonMart(stockWithLimit, speech: nil, useCat: false, discount: nil, currency: "money", cantSell: false, billEnd: false, random: nil)
  ...
  stock = getRandomStock(stock, random) if random
  getSpeech = getChosenSpeech(speech)
  commands, cmdBuy, cmdSell, cmdBill, cmdQuit = setCommands(cantSell, getSpeech, stock)
...
def setCommands(cantSell, getSpeech, stock)
  commands = []
  cmdBuy  = -1
  cmdSell = -1
  cmdBill = -1
  cmdQuit = -1
  commands[cmdBuy = commands.length]  = _INTL(getSpeech[:MenuTextBuy]&.sample || "I'm here to buy") if !$game_switches[APMSettings::BillSwitch] && !checkOutOfStock(stock)
  commands[cmdSell = commands.length] = _INTL(getSpeech[:MenuTextSell]&.sample || "I'm here to sell") if !cantSell && !$game_switches[APMSettings::BillSwitch]
  commands[cmdBill = commands.length] = _INTL(getSpeech[:MenuTextBill]&.sample || "I'm here to checkout") if $game_switches[APMSettings::BillSwitch]
