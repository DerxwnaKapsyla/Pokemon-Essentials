...
class Battle::Scene
  ...
  def pbGetDisplayEffects(battler)
    ...
    if battler.damageThreshold
      desc = _INTL("The Puppet's HP won't fall below {1}% when attacked.", battler.damageThreshold.abs)
      display_effects.push([_INTL("Damage Gate"), "--", desc])
    end
    #---------------------------------------------------------------------------
    # Raid battle shields.
    if battler.hasRaidShield?
      desc = _INTL("The Puppet is immune to status moves and takes less damage.")
    ...
    # Special states.
    if battler.mega?
      desc = _INTL("The Puppet is in its Mega Evolved form.")
      display_effects.push([_INTL("Mega Evolution"), "--", desc])
    elsif battler.primal?
      desc = _INTL("The Puppet is in its Primal form.")
      display_effects.push([_INTL("Primal Reversion"), "--", desc])
    elsif battler.ultra?
      desc = _INTL("The Puppet is in its Ultra form.")
    ...
    if !weather_data.nil? && weather_data.id != :None
      ...
      when :Sandstorm
        if battler.pbOwnedByPlayer? && battler.pbHasType?(:ROCK)
          desc = _INTL("Boosts the Sp. Defense stat of The Puppet by 50%.")
        elsif battler.pbOwnedByPlayer? && battler.takesSandstormDamage?
          desc = _INTL("The Puppet loses {1} HP each turn.", (battler.real_totalhp / 16).floor)
        else
          desc = _INTL("Puppets may lose HP each turn. Boosts Sp. Defense of Rock types.")
        end
      when :Hail
        if battler.pbOwnedByPlayer? && battler.takesHailDamage?
          desc = _INTL("The Puppet loses {1} HP each turn. Blizzard always hits.", (battler.real_totalhp / 16).floor)
        else
          desc = _INTL("Puppets may lose HP each turn. Blizzard always hits.")
        end
      when :Snow
        if battler.pbOwnedByPlayer? && battler.pbHasType?(:ICE)
          desc = _INTL("Boosts the Defense of The Puppet by 50%. Blizzard always hits.")
        else
          desc = _INTL("The move Blizzard always hits. Boosts Defense of Ice types.")
        end
      when :ShadowSky
        if battler.shadowPokemon?
          desc = _INTL("The power of The Puppet's Shadow type moves are boosted by 50%.")
        elsif battler.pbOwnedByPlayer?
          desc = _INTL("The Puppet loses {1} HP each turn.", (battler.real_totalhp / 16).floor)
        else
          desc = _INTL("Non-Shadow Puppets may lose HP each turn.")
        end
      else
    ...
    if @battle.field.terrain != :None && (!battler.pbOwnedByPlayer? || battler.affectedByTerrain?)
      ...
      when :Electric
        if battler.pbOwnedByPlayer?
          desc = _INTL("The Puppet can't fall asleep. Electric moves boosted by 30%.")
        else
          desc = _INTL("Grounded Puppets can't fall asleep. Boosts Electric moves.")
        end
      when :Grassy
        if battler.pbOwnedByPlayer?
          desc = _INTL("The Puppet heals {1} HP each turn. Grass moves boosted by 30%.", (battler.real_totalhp / 16).floor)
        else
          desc = _INTL("Grounded Puppets may recover HP each turn. Boosts Grass moves.")
        end
      when :Psychic
        if battler.pbOwnedByPlayer?
          desc = _INTL("The Puppet is immune to priority. Psychic moves boosted by 30%.")
        else
          desc = _INTL("Grounded Puppets immune to priority. Boosts Psychic moves.")
        end
      when :Misty
        if battler.pbOwnedByPlayer?
          desc = _INTL("The Puppet is immune to statuses. Dragon moves weakened.")
        else
          desc = _INTL("Grounded Puppets immune to statuses. Weakens Dragon moves.")
        end
      else
      ...
      key_hash.each do |type, effects|
        effects.each do |effect|
          ...
          when :IonDeluge
            ...
            desc = _INTL("Normal type moves of all Puppets on the field become Electric type.")
          #---------------------------------------------------------------------
          when :FairyLock
            name = GameData::Move.get(:FAIRYLOCK).name
            tick = sprintf("%d/%d", value, 2)
            desc = _INTL("No Puppets on the field can flee.")
          #---------------------------------------------------------------------
          when :Gravity
            name = GameData::Move.get(:GRAVITY).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("Grounds all Puppets. Prevents midair actions. Increases accuracy.")
          #---------------------------------------------------------------------
          when :MagicRoom
            name = GameData::Move.get(:MAGICROOM).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("No Puppets on the field can use their held items.")
          #---------------------------------------------------------------------
          when :WonderRoom
            name = GameData::Move.get(:WONDERROOM).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("All Puppets on the field swap their Defense and Sp. Defense stats.")
          #---------------------------------------------------------------------
          when :TrickRoom
            name = GameData::Move.get(:TRICKROOM).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("Reverses Speed order so slower Puppets on the field move first.")
          #---------------------------------------------------------------------
          when :MudSportField
            name = GameData::Move.get(:MUDSPORT).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("All Puppets on the field take less damage from Electric moves.")
          #---------------------------------------------------------------------
          when :WaterSportField
            name = GameData::Move.get(:WATERSPORT).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("All Puppets on the field take less damage from Fire moves.")
          ######################################################################
          #
          # TEAM EFFECTS - CHEERS
          #
          ...
          when :CheerOffense1
            ...
            desc = _INTL("Attacks of Puppets on this side deal increased damage.")
          #---------------------------------------------------------------------
          when :CheerOffense2
            name = _INTL("Offense Cheer 2")
            tick = sprintf("%d/%d", value, 3)
            desc = _INTL("Attacks Puppets on this side always trigger effects.")
          #---------------------------------------------------------------------
          when :CheerOffense3
            name = _INTL("Offense Cheer 3")
            tick = sprintf("%d/%d", value, 3)
            desc = _INTL("Attacks of Puppets on this side bypass protections.")
          #---------------------------------------------------------------------
          when :CheerDefense1
            name = _INTL("Defense Cheer 1")
            tick = sprintf("%d/%d", value, 3)
            desc = _INTL("Puppets on this side take reduced damage from attacks.")
          #---------------------------------------------------------------------
          when :CheerDefense2
            name = _INTL("Defense Cheer 2")
            tick = sprintf("%d/%d", value, 3)
            desc = _INTL("Puppets on this side are immune to move effects.")
          #---------------------------------------------------------------------
          when :CheerDefense3
            name = _INTL("Defense Cheer 3")
            tick = sprintf("%d/%d", value, 3)
            desc = _INTL("Puppets on this side will endure incoming attacks.")
          ######################################################################
          #
          ...
          when :StealthRock
            ...
            tick = _INTL("+1/1")
            desc = _INTL("Puppets on this side take Rock type damage upon entry.")
          #---------------------------------------------------------------------
          when :Steelsurge
            name = GameData::Move.get(:GMAXSTEELSURGE).name
            tick = _INTL("+1/1")
            desc = _INTL("Puppets on this side take Steel type damage upon entry.")
          #---------------------------------------------------------------------
          when :StickyWeb
            name = GameData::Move.get(:STICKYWEB).name
            tick = _INTL("+1/1")
            desc = _INTL("The Speed of grounded Puppets on this side is lowered upon entry.")
          #---------------------------------------------------------------------
          when :Spikes
            name = GameData::Move.get(:SPIKES).name
            tick = sprintf("+%d/3", value)
            desc = _INTL("Grounded Puppets on this side lose HP upon entry.")
          #---------------------------------------------------------------------
          when :ToxicSpikes
            name = GameData::Move.get(:TOXICSPIKES).name
            tick = sprintf("+%d/2", value)
            desc = _INTL("Grounded Puppets on this side are poisoned upon entry.")
          ######################################################################
          #
          ...
          when :VineLash
            ...
            desc = _INTL("Puppets on this side lose HP each turn. No effect on Grass types.")
          #---------------------------------------------------------------------
          when :Wildfire
            name = GameData::Move.get(:GMAXWILDFIRE).name
            tick = sprintf("%d/%d", value, 4)
            desc = _INTL("Puppets on this side lose HP each turn. No effect on Fire types.")
          #---------------------------------------------------------------------
          when :Cannonade
            name = GameData::Move.get(:GMAXCANNONADE).name
            tick = sprintf("%d/%d", value, 4)
            desc = _INTL("Puppets on this side lose HP each turn. No effect on Water types.")
          #---------------------------------------------------------------------
          when :Volcalith
            name = GameData::Move.get(:GMAXVOLCALITH).name
            tick = sprintf("%d/%d", value, 4)
            desc = _INTL("Puppets on this side lose HP each turn. No effect on Rock types.")
          #---------------------------------------------------------------------  
          when :SeaOfFire
          ...
          when :Rainbow
            name = _INTL("Rainbow")
            tick = sprintf("%d/%d", value, 4)
            desc = _INTL("Puppets on this side are more likely to trigger move effects.")
          #---------------------------------------------------------------------
          when :Swamp
            name = _INTL("Swamp")
            tick = sprintf("%d/%d", value, 4)
            desc = _INTL("The Speed of The Puppet on this side is halved.")
          #---------------------------------------------------------------------
          when :Tailwind
            name = GameData::Move.get(:TAILWIND).name
            tick = sprintf("%d/%d", value, 4)
            desc = _INTL("The Speed of The Puppet on this side is doubled.")
          ######################################################################
          #
          ...
          when :CraftyShield
            ...
            desc = _INTL("Puppets on this side are protected from most status moves.")
          #---------------------------------------------------------------------
          when :QuickGuard
            name = GameData::Move.get(:QUICKGUARD).name
            desc = _INTL("Puppets on this side are protected from most priority moves.")
          #---------------------------------------------------------------------
          when :WideGuard
            name = GameData::Move.get(:WIDEGUARD).name
            desc = _INTL("Puppets on this side are protected from most spread moves.")
          #---------------------------------------------------------------------
          when :MatBlock
            name = GameData::Move.get(:MATBLOCK).name
            desc = _INTL("Puppets on this side are protected from most damaging moves.")
          #---------------------------------------------------------------------
          when :AuroraVeil
            name = GameData::Move.get(:AURORAVEIL).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("All moves deal less damage to Puppets on this side.")
          #---------------------------------------------------------------------
          when :Reflect
            name = GameData::Move.get(:REFLECT).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("Physical moves deal less damage to Puppets on this side.")
          #---------------------------------------------------------------------
          when :LightScreen
            name = GameData::Move.get(:LIGHTSCREEN).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("Special moves deal less damage to Puppets on this side.")
          #---------------------------------------------------------------------
          when :Safeguard
            name = GameData::Move.get(:SAFEGUARD).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("Puppets on this side are protected from status conditions.")
          #---------------------------------------------------------------------
          when :Mist
            name = GameData::Move.get(:MIST).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("The stats of Puppets on this side cannot be lowered.")
          #---------------------------------------------------------------------
          when :LuckyChant
            name = GameData::Move.get(:LUCKYCHANT).name
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("Puppets on this side are immune to critical hits.")
          ######################################################################
          #
          # POSITION EFFECTS
          #
          ...
          when :FutureSightCounter
            ...
            desc = _INTL("{1} will strike The Puppet in this spot in {2} more turn(s).", GameData::Move.get(data).name, value)
          #---------------------------------------------------------------------
          when :Wish
            name = GameData::Move.get(:WISH).name
            tick = value.to_s
            data = (battler.pbOwnedByPlayer?) ? @battle.positions[battler.index].effects[PBEffects::WishAmount] : "???"
            desc = _INTL("The Puppet in this spot will restore {1} HP in {2} more turn(s).", data, value)
          #---------------------------------------------------------------------
          when :HealingWish
            name = GameData::Move.get(:HEALINGWISH).name
            desc = _INTL("Fully heals a Puppets switching into this spot.")
          #---------------------------------------------------------------------
          when :LunarDance
          ...
          when :ZHealing
            ...
            desc = _INTL("A Puppets switching into this spot will recover its HP.")
          ######################################################################
          #
          ...
          when :Substitute
            ...
            desc = _INTL("A substitute with {1} HP stands in for The Puppet.", value)
          #---------------------------------------------------------------------
          when :AquaRing
            ...
            desc = _INTL("The Puppet restores {1} HP at the end of each turn.", data)
          #---------------------------------------------------------------------
          when :Ingrain
            ...
            desc = _INTL("The Puppet restores {1} HP every turn, but cannot escape.", data)
          #---------------------------------------------------------------------
          when :Toxic
          ...
          when :LeechSeed
            ...
            desc = _INTL("Each turn, {1} leeches {2} HP from The Puppet.", @battle.battlers[value].pbThis(true), data)
          #---------------------------------------------------------------------
          when :Curse
            name = GameData::Move.get(:CURSE).name
            data = (battler.pbOwnedByPlayer?) ? (battler.real_totalhp / 4).floor : "some"
            desc = _INTL("The Puppet loses {1} HP at the end of each turn.", data)
          #---------------------------------------------------------------------
          when :Nightmare
            name = GameData::Move.get(:NIGHTMARE).name
            data = (battler.pbOwnedByPlayer?) ? (battler.real_totalhp / 4).floor : "some"
            desc = _INTL("The Puppet loses {1} HP each turn it spends asleep.", data)
          #---------------------------------------------------------------------
          when :SaltCure
          ...
          when :Splinters
            ...
            if battler.effects[PBEffects::SplintersType]
              desc = _INTL("The Puppet takes {1} type damage at the end of each turn.", 
          ...
          when :HealBlock
            ...
            tick = sprintf("%d/%d", value, 5)
            desc = _INTL("The Puppet cannot use any healing effects that restores HP.")
          #---------------------------------------------------------------------
          when :Attract
            name = _INTL("Infatuation")
            desc = _INTL("The Puppet is less likely to attack {1}.", @battle.battlers[value].pbThis(true))
          #---------------------------------------------------------------------
          when :Confusion
            name = _INTL("Confusion")
            tick = _INTL("?/?")
            desc = _INTL("The Puppet may hurt itself in confusion instead of attacking.")
          #---------------------------------------------------------------------
          when :Outrage
            name = _INTL("Rampaging")
            tick = _INTL("?/?")
            desc = _INTL("The Puppet rampages for a few turns. It then becomes confused.")
          #---------------------------------------------------------------------
          when :Torment
            name = GameData::Move.get(:TORMENT).name
            desc = _INTL("The Puppet cannot use the same move twice in a row.")
          #---------------------------------------------------------------------
          when :ThroatChop
            name = GameData::Move.get(:THROATCHOP).name
            tick = sprintf("%d/%d", value, 2)
            desc = _INTL("The Puppet cannot use any sound-based moves.")
          #---------------------------------------------------------------------
          when :Encore
            name = GameData::Move.get(:ENCORE).name
            tick = sprintf("%d/%d", value, 3)
            data = GameData::Move.get(battler.effects[PBEffects::EncoreMove]).name
            desc = _INTL("The Puppet may only use the move {1}.", data)
          #---------------------------------------------------------------------
          when :Disable
            name = _INTL("Move Disabled")
            tick = sprintf("%d/%d", value, 4)
            data = GameData::Move.get(battler.effects[PBEffects::DisableMove]).name
            desc =_INTL("The Puppet cannot use the move {1}.", data)
          #---------------------------------------------------------------------
          when :Taunt
            name = GameData::Move.get(:TAUNT).name
            tick = sprintf("%d/%d", value, 4)
            desc = _INTL("The Puppet cannot use any status moves.")
          #---------------------------------------------------------------------
          when :GastroAcid
            name = _INTL("No Ability")
            desc = _INTL("The Puppet's ability is negated.")
          #---------------------------------------------------------------------
          when :Embargo
          ...
          when :Charge
            ...
            desc = _INTL("The next Electric move used by The Puppet will double in power.")
          #---------------------------------------------------------------------
          when :Electrify
            name = GameData::Move.get(:ELECTRIFY).name
            desc = _INTL("The next move used by The Puppet will become Electric type.")
          #---------------------------------------------------------------------
          when :MagnetRise
          ...
          when :Powder
            ...
            desc = _INTL("The next Fire move used by The Puppet will ignite and explode.")
          #---------------------------------------------------------------------
          when :TarShot
          ...
          when :SmackDown
            ...
            if battler.pbOwnedByPlayer? && battler.pbHasType?(:FLYING)
              desc = _INTL("The Puppet is grounded and no longer immune to Ground moves.")
            else
          ...
          when :Foresight
            ...
            if battler.pbOwnedByPlayer? && battler.pbHasType?(:GHOST)
              desc = _INTL("The Puppet's evasion boosts are ignored. No Ghost immunities.")
            else
          ...
          when :MiracleEye
            ...
            if battler.pbOwnedByPlayer? && battler.pbHasType?(:DARK)
              desc = _INTL("The Puppet's evasion boosts are ignored. No Dark immunities.")
            else
              desc = _INTL("The Puppet's evasion boosts are ignored.")
            end
          ######################################################################
          ...
          when :Rage
            ...
            desc = _INTL("The Attack stat of The Puppet increases whenever it is hit.")
          #---------------------------------------------------------------------
          when :HelpingHand
            name = GameData::Move.get(:HELPINGHAND).name
            desc = _INTL("The damage dealt with The Puppet's next move will be increased.")
          #---------------------------------------------------------------------
          when :PowerTrick
            name = GameData::Move.get(:POWERTRICK).name
            desc = _INTL("The Attack and Defense stats of The Puppet are swapped.")
          #---------------------------------------------------------------------
          when :LaserFocus
            name = GameData::Move.get(:LASERFOCUS).name
            tick = sprintf("%d/%d", value, 2)
            desc = _INTL("The Puppet's next attack will be a guaranteed critical hit.")
          #---------------------------------------------------------------------
          when :Stockpile
            name = GameData::Move.get(:STOCKPILE).name
            tick = sprintf("+%d/%d", value, 3)
            desc = _INTL("The Puppet is amassing a stockpile to use or consume.")
          #---------------------------------------------------------------------
          when :Minimize
            name = GameData::Move.get(:MINIMIZE).name
            desc = _INTL("The Puppet takes more damage from certain moves while minimized.")
          #---------------------------------------------------------------------
          when :GlaiveRush
            name = _INTL("Vulnerable")
            tick = sprintf("%d/%d", value, 2)
            desc = _INTL("The Puppet cannot evade and takes double damage from attacks.")
          #---------------------------------------------------------------------
          when :Telekinesis
          ...
          when :LockOn
            ...
            desc = _INTL("The Puppet's next move is sure to hit {1}.", data.pbThis(true))
          #---------------------------------------------------------------------
          when :Syrupy
            name = _INTL("Speed Down")
            tick = sprintf("%d/%d", value, 3)
            data = @battle.battlers[battler.effects[PBEffects::SyrupyUser]]
            desc = _INTL("Each turn, {1} lowers The Puppet's Speed.", data.pbThis(true), value)
          #---------------------------------------------------------------------
          when :WeightChange
          ...
          when :NoRetreat
            ...
            desc = _INTL("The Puppet refuses to flee or switch out.")
          #---------------------------------------------------------------------
          when :JawLock
          ...
          when :MagicCoat
            ...
            desc = _INTL("The Puppet bounces back most incoming status moves.")
          #---------------------------------------------------------------------
          when :Protect
            name = GameData::Move.get(:PROTECT).name
            desc = _INTL("The Puppet is protected from most incoming attacks.")
          #---------------------------------------------------------------------
          when :SpikyShield
          ...
          when :TwoTurnAttack 
            ...
            desc = _INTL("The Puppet will use {1} next turn.", GameData::Move.get(value).name)
            if battler.semiInvulnerable?
              display_effects.push([_INTL("Semi-Invulnerable"), "--", _INTL("The Puppet cannot be hit by most attacks.")])
            end
          #---------------------------------------------------------------------
          when :SkyDrop
            name = GameData::Move.get(:SKYDROP).name
            desc = _INTL("The Puppet is being lifted in the air by {1}.", @battle.battlers[value].pbThis(true))
          #---------------------------------------------------------------------
          when :HyperBeam
            name = _INTL("Recharging")
            tick = value.to_s
            desc = _INTL("The Puppet cannot act for {1} more turn(s).", value)
          #---------------------------------------------------------------------
          when :Yawn
            name = _INTL("Drowsy")
            tick = value.to_s
            desc = _INTL("The Puppet will fall asleep in {1} more turn(s).", value)
          #---------------------------------------------------------------------
          when :PerishSong
            name = _INTL("Perish Count")
            tick = value.to_s
            desc = _INTL("The Puppet will be forced to faint in {1} more turn(s).", value)
          #---------------------------------------------------------------------
          when :SlowStart
            next if !battler.hasActiveAbility?(:SLOWSTART)
            name = GameData::Ability.get(:SLOWSTART).name
            tick = value.to_s
            desc = _INTL("The Puppet gets its act together in {1} more turn(s).", value)
          #---------------------------------------------------------------------
          when :Miasma
    ...
    @battle.allBattlers.each do |b|
      ...
      name = GameData::Move.get(:JAWLOCK).name
      desc = _INTL("The Puppet is trapped while latched to {1}.", b.pbThis(true))
    ...
    @battle.allOtherSideBattlers(battler.index).each do |b|
      ...
      desc = _INTL("The Puppet can't use moves known by {1}.", b.pbThis(true))
      display_effects.push([name, "--", desc])
      break
    end
    #---------------------------------------------------------------------------
