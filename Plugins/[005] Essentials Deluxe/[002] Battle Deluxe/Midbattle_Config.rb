...
################################################################################
  ...
  #-----------------------------------------------------------------------------
    "turnCommand" => {
      ...
      :effects 		=> [ [PBEffects::Endure, true] ],
	  :battler_1 	=> :Self,
	  :effects_1	=> [ [PBEffects::Endure, true] ]
	},
  #-----------------------------------------------------------------------------
  # Condition: When Poliwhirl is low on health, restore HP to full (Repeat)
  #-----------------------------------------------------------------------------
	"defenderHPLow_repeat" => {
	  :speech		=> [0, "C'mon {1}, don't give up now!"],
	  :battler 		=> :Self,
	  :text    		=> "{1} got a second wind!",
	  :hp      		=> 1 
	},
  
  #-----------------------------------------------------------------------------
  # Condition: When Nidorino is low on HP, have Red throw a Pokeball and capture
  #			   Nidorino.
  #-----------------------------------------------------------------------------
    "turnEnd_repeat" => {
	  :delay   		=> "defenderHPLow_foe",
      :speech		=> [0, "And now that Nidorino's weak, I throw the Poké Ball, and...!"],
      :useitem 		=> :POKEBALL,
    },
