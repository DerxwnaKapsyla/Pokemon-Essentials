...
class Battle::Scene
  def dx_midbattle(idxBattler, idxTarget, *triggers)
    ...
    triggers.each do |trigger|
      ...
      if trig.length > 1 && ["turnCommand", "turnAttack", "turnEnd"].include?(trig[0])
        ...
        if !trigger.include?("_repeat")
          ...
          all_triggers.push(trigger + "_repeat_three")
		  all_triggers.push(trigger + "_repeat_five")
          all_triggers.push(trigger + "_repeat_six")
		  all_triggers.push(trigger + "_repeat_ten")
        end
      end
    end
    #---------------------------------------------------------------------------
    ...
    all_triggers.each do |trigger|
	  ...
	  next if trigger.include?("repeat_three") && ((@battle.turnCount + 1) % 3 != 0 || !@battle.turnCount.even?)
	  next if trigger.include?("repeat_five") && ((@battle.turnCount + 1) % 5 != 0 || !@battle.turnCount.even?)
	  next if trigger.include?("repeat_six") && (@battle.turnCount + 1) % 6 != 0
	  next if trigger.include?("repeat_ten") && (@battle.turnCount + 1) % 10 != 0
      ...
      when Hash
        ...
        keys.each do |key|
          ...
          # Changes BGM - No wait.
          #---------------------------------------------------------------------
          when :bgmnw, :musicnw		  then pbBGMPlay(value)
          #---------------------------------------------------------------------
          # Play ME
          #---------------------------------------------------------------------
          when :me					  then pbMEPlay(value)
		  #---------------------------------------------------------------------
          # Stop BGM.
          #---------------------------------------------------------------------
          when :nobgm, :nomusic		  then pbBGMFade(2.0)
          #---------------------------------------------------------------------
		  ...
		  when :backdrop2			  then midbattle_ChangeBackdrop2(value)
          #---------------------------------------------------------------------
          # Toggle a switch on or off
          #---------------------------------------------------------------------
		  when :switchon			  then $game_switches[value] = true
		  when :switchoff			  then $game_switches[value] = false
          #---------------------------------------------------------------------
  ...
  def midbattle_ChangeAbility(battler, value)
	...
	if GameData::Ability.exists?(abil) && battler.ability != abil
  ...
  def midbattle_ChangeStats(battler, value)
    ...
    #---------------------------------------------------------------------------
    # Resets all of a battler's defensive stats
	when :Reset_Defenses
      showMsg = false
	  ...
	  @battle.pbDisplay(_INTL("{1}'s defenses returned to normal!", battler.pbThis)) if showMsg
    end
  end
  ...
  #-------------------------------------------------------------------------------
  # Changes the backdrop, with a faster flash and a sound effect
  #-------------------------------------------------------------------------------
  def midbattle_ChangeBackdrop2(value)
    return if @battle.decision > 0
    if value.is_a?(Array)
      backdrop, base = value[0], value[1]
    else
