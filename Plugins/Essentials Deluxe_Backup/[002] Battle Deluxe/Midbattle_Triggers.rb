  ...
  def pbHitEffectivenessMessages(user, target, numTargets = 1)
    ...
    if !target.damageState.substitute
      ...
      if !target.fainted? && user.opposes?(target.index)
        ...
        #      target_triggers.push((target.pbOwnedByPlayer?) ? "oneHP_final" : (target.opposes?) ? "oneHP_final_foe" : "oneHP_final_ally")
        #    else
        #      target_triggers.push((target.pbOwnedByPlayer?) ? "oneHP" : (target.opposes?) ? "oneHP_foe" : "oneHP_ally")
        #    end
        #  else
        #    target_triggers.push((target.pbOwnedByPlayer?) ? "oneHP" : (target.opposes?) ? "oneHP_foe" : "oneHP_ally")
        #    target_triggers.push((target.pbOwnedByPlayer?) ? "oneHP_final" : (target.opposes?) ? "oneHP_final_foe" : "oneHP_final_ally")
        ...
        #  if @battle.pbParty(target.index).length > @battle.pbSideSize(target.index)
        #    if @battle.pbAbleNonActiveCount(target.index) == 0
        #      target_triggers.push((target.pbOwnedByPlayer?) ? "twoHP_final" : (target.opposes?) ? "twoHP_final_foe" : "twoHP_final_ally")
        #    else
        #      target_triggers.push((target.pbOwnedByPlayer?) ? "twoHP" : (target.opposes?) ? "twoHP_foe" : "twoHP_ally")
        #    end
        #  else
        #    target_triggers.push((target.pbOwnedByPlayer?) ? "twoHP" : (target.opposes?) ? "twoHP_foe" : "twoHP_ally")
        #    target_triggers.push((target.pbOwnedByPlayer?) ? "twoHP_final" : (target.opposes?) ? "twoHP_final_foe" : "twoHP_final_ally")
