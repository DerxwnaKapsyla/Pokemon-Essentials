#===============================================================================
# Single Trainer Battle - Animated Background
#===============================================================================
SpecialBattleIntroAnimations.register("vs_boss_solo_anim_bg", 200,   # Priority 80
  proc { |battle_type, foe, location|   # Condition
	...
	next false if !$game_temp.should_bg_animate
  ...
  proc { |viewport, battle_type, foe, location|   # Animation
	...
	ball_bar.z       = 99998
    # Background Graphic
    bg_animated             = AnimatedBitmap.new("Graphics/Transitions/#{bg_graphic}")
	background              = Sprite.new(viewport)
    background.bitmap       = bg_animated.bitmap
    background.z            = 99990
...
#===============================================================================
# Double Trainer Battle - Animated Background
#===============================================================================
SpecialBattleIntroAnimations.register("vs_boss_duo_anim_bg", 201,   # Priority 81
  proc { |battle_type, foe, location|   # Condition
    next false if ![1, 3].include?(battle_type)   # Trainer battles only
	next false if foe.length == 1 # Multi-Battles Only
    tr_type   = foe[0].trainer_type
...
#===============================================================================
# Single Wild Battle - Ayakashi
# Aka, do not move Puppet portrait
#===============================================================================
SpecialBattleIntroAnimations.register("vs_ayakashi", 90,   # Priority 100
  proc { |battle_type, foe, location|   # Condition
    next false if ![0, 2].include?(battle_type)   # Wild Encounters Only
    species = foe[0].species
	form    = foe[0].form
	base_path = "Graphics/Transitions/DTS/PTs"
	next false if species == !:AYAKASHI
...
#===============================================================================
# Single Trainer Battle - Vs. Renko or Maribel, End of Act 1 Variant
# Used to display a different portrait for them- I'm not making a separate
# trainer class JUST for that.
#===============================================================================
SpecialBattleIntroAnimations.register("vs_boss_solo", 90,   # Priority 80
  proc { |battle_type, foe, location|   # Condition
	...
	next false if foe.length != 1 # Multi-Battles Only
    tr_type  = foe[0].trainer_type
	next false if ![:SCRenko, :SCMary].include?(tr_type)
    next pbResolveBitmap("Graphics/Transitions/DTS/PTs/Char_#{tr_type}") # Character cut-in
  },
  proc { |viewport, battle_type, foe, location|   # Animation
	#$game_temp.vs_name = dkGetTrainerName(foe)
    ...
    bg_name        = $game_temp.get_vs_transition_bg
    bg_graphic     = sprintf("DTS/BGs/BG%s", bg_name.to_s) rescue nil
    tr_graphic     = sprintf("DTS/PTs/Char_%s_Alt", tr_type.to_s) rescue nil
    black_bars     = sprintf("DTS/BorderBars") rescue nil
    # Set up sprites
	ball_sprites     = dkDisplayBallCount(viewport, foe, 0) # Create the ball count for a trainer
	ball_bar         = Sprite.new(viewport)
