...
class TriadCard
  def price
    ...
    if ret > 10000;   ret = (1 + ret / 1000) * 500
    elsif ret > 5000; ret = (1 + ret / 500) * 250
    elsif ret > 1000; ret = (1 + ret / 100) * 50
    elsif ret > 500;  ret = (1 + ret / 50) * 25
...
def pbBuyTriads
  ...
  goldwindow = Window_UnformattedTextPokemon.newWithSize(
     _INTL("Coins:\r\n{1}",$Trainer.coins.to_s_formatted),0,0,32,32)
  ...
  loop do
    ...
    if Input.trigger?(Input::BACK)
      ...
      maxafford = (price<=0) ? 99 : $Trainer.coins.to_s_formatted/price
      ...
      $Trainer.coins -= price
      goldwindow.text = _INTL("Coins:\r\n{1}",$Trainer.coins.to_s_formatted)
      ...
      if Input.trigger?(Input::USE)
        ...
        if quantity>0
          ...
          if pbConfirmMessage(_INTL("I can pay {1} Coins. Would that be OK?",price.to_s_formatted))
            ...
            pbMessage(_INTL("Turned over the {1} card and received {2} Coins.\\se[Mart buy item]",itemname,$Trainer.coins.to_s_formatted))
            commands = []
            for i in 0...$PokemonGlobal.triads.length
              item = $PokemonGlobal.triads[i]
              speciesname = GameData::Species.get(item[0]).name
