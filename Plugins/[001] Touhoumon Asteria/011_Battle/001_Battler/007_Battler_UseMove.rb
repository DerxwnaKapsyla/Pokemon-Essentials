...
class PokeBattle_Battler
  def pbEndTurn(_choice)
    ...
    if !@effects[PBEffects::ChoiceBand] &&
       hasActiveItem?([:CHOICEBAND,:CHOICESPECS,:CHOICESCARF,:BLOOMERS,:POWERRIBBON])
...
#==============================================================================#
  def pbUseMove(choice,specialUsage=false)
    ...
    # Powder
    if user.effects[PBEffects::Powder] && (move.calcType == :FIRE || move.calcType == :FIRE18)
      @battle.pbCommonAnimation("Powder",user)
	  battle.pbDisplay(_INTL("The flame-touched powder went and exploded!"))
      #battle.pbDisplay(_INTL("When the flame touched the powder on the Pokémon, it exploded!")) # Derx: Original Line
      user.lastMoveFailed = true
