...
class PurifyChamberScreen
  def pbPlace(pkmn,position)
    ...
    if (position==0)
      if pkmn.shadowPokemon?
        ...
        @scene.pbDisplay(_INTL("Only a Shadow Pokémon or a Puppet can go there."))
        return false
      end
    elsif (position>=1)
      if pkmn.shadowPokemon?
        @scene.pbDisplay(_INTL("Can't place a Shadow Pokémon or a Puppet there."))
        return false
      else
        oldpkmn=PurifyChamberHelper.pbGetPokemon(@chamber,position)
        if oldpkmn
  ...
  def pbOnPlace(pkmn)
    ...
    if @chamber.setCount(set)==0 && @chamber.isPurifiableIgnoreRegular?(set)
      ...
      @scene.pbDisplay(
         _INTL("This {1} is ready to open its heart. However, there must be at least one regular party member in the set to perform a purification ceremony.",pkmn.name))
    end
  end
  ...
  def pbOpenSetDetail
    ...
    loop do
      ...
      if cmd[0]==0
        ...
        if heldpkmn
          @scene.pbDisplay("You're holding something!")
        else
