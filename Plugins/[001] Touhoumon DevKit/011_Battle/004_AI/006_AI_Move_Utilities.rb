...
# Derx: Added a check for Scrappy w/ Thmn Ghost
      ret = Effectiveness::NORMAL_EFFECTIVE_ONE if (defType == :GHOST || defType == :GHOST18) &&
                                                   Effectiveness.ineffective_type?(moveType, defType)
    end
    # Miracle Eye
    if target.effects[PBEffects::MiracleEye]
# Derx: Added a check for Miracle Eye w/ Thmn Dark
      ret = Effectiveness::NORMAL_EFFECTIVE_ONE if (defType == :DARK || defType == :DARK18) &&
                                                   Effectiveness.ineffective_type?(moveType, defType)
    end
    # Delta Stream's weather
    if @battle.pbWeather == :StrongWinds
# Derx: Added a check for Strong Winds w/ Thmn Flying
      ret = Effectiveness::NORMAL_EFFECTIVE_ONE if (defType == :FLYING || defType == :FLYING18) &&
                                                   Effectiveness.super_effective_type?(moveType, defType)
    end
    # Grounded Flying-type Pokémon become susceptible to Ground moves
    if !target.airborne?
# Derx: Added a check for Airborne Targets w/ Thmn Flying/Earth
      ret = Effectiveness::NORMAL_EFFECTIVE_ONE if (defType == :FLYING || defType == :FLYING18) && 
...
# Derx: Added a check for Iron Ball w/ Thmn Earth/Flying
    return Effectiveness::NORMAL_EFFECTIVE if (moveType == :GROUND || moveType == :EARTH18) &&
       (target.pbHasType?(:FLYING) || target.pbHasType?(:FLYING18)) && target.hasActiveItem?(:IRONBALL)
    # Determine types
  ...
  def pbCheckMoveImmunity(score,move,user,target,skill)
    ...
    if skill>=PBTrainerAI.mediumSkill
      case type
      when :GROUND, :EARTH18 # Derx: Added a check for Thmn Earth
        return true if target.airborne? && !move.hitsFlyingTargets?
      when :FIRE, :FIRE18 # Derx: Added a check for Thmn Fire
        return true if target.hasActiveAbility?(:FLASHFIRE)
      when :WATER, :WATER18 # Derx: Added a check for Thmn Water
        return true if target.hasActiveAbility?([:DRYSKIN,:STORMDRAIN,:WATERABSORB])
      when :GRASS, :NATURE18 # Derx: Added a check for Thmn Nature
        return true if target.hasActiveAbility?(:SAPSIPPER)
      when :ELECTRIC, :WIND18 # Derx: Added a check for Thmn Wind
        return true if target.hasActiveAbility?([:LIGHTNINGROD,:MOTORDRIVE,:VOLTABSORB])
      end
      return true if Effectiveness.not_very_effective?(typeMod) &&
                     target.hasActiveAbility?(:WONDERGUARD)
# Derx: Added a check for Play Ghost
      return true if Effectiveness.not_very_effective?(typeMod) &&
                     target.hasActiveAbility?(:PLAYGHOST)
...
# Derx: Added a check for Thmn Nature
		#return true if target.pbHasType?(:NATURE18) # Derx: This change is commented out because it isn't official in 1.8 and may shake things up too much
...
# Derx: Added a check for Prankster w/ Thmn Dark
      return true if Settings::MECHANICS_GENERATION >= 7 && user.hasActiveAbility?(:PRANKSTER) &&
                     target.pbHasType?(:DARK18) && target.opposes?(user)
  ...
  def pbRoughDamage(move,user,target,skill,baseDmg)
    ...
    # Charge
    if skill>=PBTrainerAI.mediumSkill
# Derx: Added a check for Charge effect w/ Thmn Wind
      if user.effects[PBEffects::Charge]>0 && (type == :ELECTRIC || type == :WIND18)
        multipliers[:base_damage_multiplier] *= 2
      end
    end
    # Mud Sport and Water Sport
    if skill>=PBTrainerAI.mediumSkill
# Derx: Added a check for Mud Sport w/ Thmn Wind
...
# Derx: Added a check for Water Sport w/ Thmn Fire
...
# Derx: Added a check for Misty Terrain w/ Thmn Faith
    ...
    if skill>=PBTrainerAI.mediumSkill
      ...
      when :Sun, :HarshSun
        if (type == :FIRE || type == :FIRE18) # Derx: Added a check for Harsh Sun w/ Thmn Fire
          multipliers[:final_damage_multiplier] *= 1.5
        elsif (type == :WATER || type == :WATER18) # Derx: Added a check for Harsh Sun w/ Thmn Water
          multipliers[:final_damage_multiplier] /= 2
        end
      when :Rain, :HeavyRain
        if  (type == :FIRE || type == :FIRE18) # Derx: Added a check for Heavy Rain w/ Thmn Fire
          multipliers[:final_damage_multiplier] /= 2
        elsif (type == :WATER || type == :WATER18) # Derx: Added a check for Heavy Rain w/ Thmn Water
          multipliers[:final_damage_multiplier] *= 1.5
        end
      when :Sandstorm
# Derx: Added a check for Sandstorm w/ Thmn Beast
        if (target.pbHasType?(:ROCK) || target.pbHasType?(:BEAST18)) && 
    ...
    if skill>=PBTrainerAI.highSkill
      if move.function=="006"   # Toxic
        modifiers[:base_accuracy] = 0 if Settings::MORE_TYPE_EFFECTS && move.statusMove? &&
                                         (user.pbHasType?(:POISON) || user.pbHasType?(:MIASMA18)) # Derx: Added a check for Miasma w/ Toxic
      end
