...
class PokemonPokedexInfo_Scene
  def pbStartScene(dexlist, index, region)
    ...
    @typebitmap = AnimatedBitmap.new(_INTL("Graphics/Pictures/Pokedex/icon_types_big")) # Derx: Changed
    @sprites = {}
  ...
  def drawPageInfo
    ...
    if @show_battled_count
      ...
      if species_data.has_flag?("Puppet")
		textpos.push([_INTL("Cost"), 314, 196, 0, base, shadow])
      else
    ...
    if $player.owned?(@species)
      # Write the category
      if species_data.has_flag?("Puppet")
		textpos.push([_INTL("{1} Puppet", species_data.category), 246, 80, 0, base, shadow])
      else
      ...
      if !@show_battled_count
        ...
        if species_data.has_flag?("Puppet")
          ...
          textpos.push([_ISPRINTF("{1:.1f}  ", weight / 10.0), 482, 196, 1, base, shadow])
        else
      ...
      # Write the category
      if species_data.has_flag?("Puppet")
      	textpos.push([_INTL("????? Puppet"), 246, 80, 0, base, shadow])
      else
      ...
      if !@show_battled_count
          ...
          textpos.push([_INTL("????.?  "), 482, 196, 1, base, shadow])
        else
          if System.user_language[3..4] == "US"   # If the user is in the United States
