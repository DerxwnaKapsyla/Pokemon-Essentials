...
# ------ Derx: Changes necessary for Pokedex screen fixes
      @xgap = 62; @ygap = 44 # Derx: @xgap was initially 124
...
#          self.src_rect.y = 44; self.src_rect.height = 44 # Derx: These were the initial lines for the above section
        when 3,4   # Height, weight
...
# ------ Derx: Put this section back in if things go haywire and work again
    ...
    when 2   # Type
      ...
      for i in 0...cmds.length
        typerect.y = @typeCommands[i].id_number*28 # Derx: Changes necessary for Pokedex screen fixes
    ...
    loop do
      ...
      if Input.trigger?(Input::UP)
        ...
        when 8   # Start search (filter)
          ...
          if dexlist.length==0
            pbMessage(_INTL("Nothing that matched was found."))
          else
            @dexlist = dexlist
            @sprites["pokedex"].commands = @dexlist
