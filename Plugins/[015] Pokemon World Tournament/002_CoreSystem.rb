...
# will not show up on your tournament selection list.
  ...
  def displayScoreboard(trainer)
    ...
    for i in 0...@trainer_list_int.length
      ...
      text=[[trname,34+(gwidth-64)*x,38+(gheight/6)*y,x*1,Color.new(255,255,255,opacity),Color.new(80,80,80,opacity_shadow)]]
  ...
  def introduction
    pbMessage(_INTL("Let's get you signed up for a tournament!"))
  ...
  def startTournament
	...
	pbSEPlay("Applause")
    pbMessage(_INTL("\\xn[Announcer]Welcome back to the Festival Tournament!\\1"))
    pbMessage(_INTL("\\xn[Announcer]Joining us in this bracket are 8 very eager contestants, all aiming for the top!\\1"))
    pbMessage(_INTL("\\xn[Announcer]Let us turn our attention to the scoreboard, to see who will be competing today.\\1"))
    trainer = self.generateRound1
    self.displayScoreboard(trainer)
    self.moveSwitch('A')
    pbMessage(_INTL("\\xn[Announcer]Without further ado, let the first match begin.\\1"))
	pbSEPlay("Applause")
    pbMessage(_INTL("\\xn[Announcer]This will be a battle between {1} and {2}.",$player.name,trainer.name))
    self.visualRound(trainer)
    pbMessage(trainer.beforebattle) if !trainer.beforebattle.nil?
	$PokemonGlobal.nextBattleBGM = "B-016. The Ground's Color is Yellow (BW Remix)"
	$PokemonGlobal.nextBattleVictoryBGM = "U-001. Victory! Vs Trainer"
    if pbPWTBattle(trainer,@challenge_rules,@outcome)
      @round = 1
      pbMessage(trainer.afterbattle) if !trainer.afterbattle.nil?
      @beat.push(trainer)
	  pbSEPlay("Applause")
      pbMessage(_INTL("\\xn[Announcer]Wow! What an exciting first round!\\1"))
      pbMessage(_INTL("\\xn[Announcer]The arena is getting heated up, and the contestants are on fire!\\1"))
      self.visualRound(trainer,true)
      pbMessage(_INTL("\\xn[Announcer]Let us turn our attention back to the scoreboard for the results.\\1"))
      trainer = self.generateRound2
      self.displayScoreboard(trainer)
      pbMessage(_INTL("\\xn[Announcer]It looks like the next match will be between {1} and {2}.\\1",$player.name,trainer.name))
	  pbSEPlay("Applause")
      self.visualRound(trainer)
      pbMessage(trainer.beforebattle) if !trainer.beforebattle.nil?
      pbMessage(_INTL("\\xn[Announcer]Let the battle begin!"))
	  $PokemonGlobal.nextBattleBGM = "B-016. The Ground's Color is Yellow (BW Remix)"
	  $PokemonGlobal.nextBattleVictoryBGM = "U-001. Victory! Vs Trainer"
      if pbPWTBattle(trainer,@challenge_rules,@outcome)
        @round = 2
        ...
        @beat.push(trainer)
		pbSEPlay("Applause")
        pbMessage(_INTL("\\xn[Announcer]What spectacular matches!\\1"))
        pbMessage(_INTL("\\xn[Announcer]These trainers are really giving it all.\\1"))
        self.visualRound(trainer,true)
        pbMessage(_INTL("\\xn[Announcer]Let's direct our attention at the scoreboard one final time.\\1"))
        ...
        if pbPWTBattle(trainer,@challenge_rules,@outcome)
          ...
          @beat.push(trainer)
		  pbSEPlay("Applause")
          pbMessage(_INTL("\\xn[Announcer]What an amazing battle!\\1"))
          pbMessage(_INTL("\\xn[Announcer]Both the trainers put up a great fight, but our very own {1} was the one to come out on top!\\1",$player.name))
          pbMessage(_INTL("\\xn[Announcer]Congratulations {1}! You have certainly earned today's title of \"Champion\"!\\1",$player.name))
		  pbSEPlay("Applause")
          pbMessage(_INTL("\\xn[Announcer]That's all we have time for. I hope you enjoyed today's contest. And we hope to see you again soon."))
          return "win"
        end
      end
    end
