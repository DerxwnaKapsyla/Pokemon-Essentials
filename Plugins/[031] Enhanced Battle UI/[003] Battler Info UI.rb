...
class Battle::Scene
  ...
  def pbGetDisplayEffects(battler)
    ...
    if !weather_data.nil? && weather_data.id != :None
      ...
      case id
      when :Sun         then desc = _INTL("Fire and Pyro type moves deal 50% more damage. Halves Water and Hydro move damage.")
      when :HarshSun    then desc = _INTL("Fire and Pyro type moves deal 50% more damage. Negates Water and Hydro type moves.")
      when :Rain        then desc = _INTL("Water and Hydro type moves deal 50% more damage. Halves Fire and Pyro move damage.")
      when :HeavyRain   then desc = _INTL("Water and Hydro type moves deal 50% more damage. Negates Fire and Pyro type moves.")
      when :StrongWinds then desc = _INTL("All Flying and Aero-type weaknesses are suppressed.")
      when :Hailstorm   then desc = _INTL("The combined effects of both Hail and Snow.")
      when :Sandstorm
        if battler.pbOwnedByPlayer? && (battler.pbHasType?(:ROCK) || battler.pbHasType?(:BEAST18))
          ...
          desc = _INTL("Pokémon may lose HP each turn. Boosts Sp. Defense of Rock and Beast types.")
        end
      when :Hail
      ...
      when :Snow
        if battler.pbOwnedByPlayer? && (battler.pbHasType?(:ICE) || battler.pbHasType?(:ICE18))
          desc = _INTL("Boosts the Defense of the Pokémon by 50%. Blizzard always hits.")
        else
          desc = _INTL("The move Blizzard always hits. Boosts Defense of Ice and Cryo types.")
        end
      when :ShadowSky
