...
################################################################################
  ...
  RED_VS_CHEN = {
	"turnCommand" => {
	  :blankspeech		=> ["PROFESSOR OAK: Red! Remember what I told you! That Reimu Puppet is just like a Pokémon!", 
							"Command her and she'll fight alongside you!"],
	  },
	"turnCommand_repeat"	=> {
	  :battler	=> 0,
	  :effect	=> [ [PBEffects::Endure, true] ]
	  },
	"defenderHPLow_repeat"		  => {
      :battler => 0,
  ...
  VS_DLWRUUKOTO = {
	"turnCommand" => {
	  :battler				=> :Opposing,
	  :speech				=> "Remember, my Master is watching. Be sure to put on a good show for him!"
	},
	"switchInAIDIALGA_foe" => {
	  :ignore				=> "switchInAIDIALGA_foe",
	  :speech				=> ["Behold what my Master is capable of!",
								"Bare witness to the Dark Metamorphosis in action!"]
	},
	"switchInLastAIDIALGA_foe" => {
	  :ignore				=> "switchInLastAIDIALGA_foe",
	  :speech				=> ["Behold what my Master is capable of!",
								"Bare witness to the Dark Metamorphosis in action!"]
	},
  }
  ...
  VS_CELESTE_CINNABAR = {
	#------------------------------
	# Start of Battle
	# Boost AHina's Physical Attack by +2
	#------------------------------
	"turnCommand" => {
	  :battler				=> :Opposing,
	  :speech				=> ["You will learn what it means to truly fear Team Rocket when I'm done with you!",
								"Time to put the genius of Team Rocket's R&D Department to use!"],
	  :text					=> ["Celeste pulls out a device and attaches it to {1}!",
								"{1} was overwhelmed by a surge of electricity!"],
	  :anim      			=> :CHARGE,
	  :stats   				=> [:ATTACK, 2],
	  :text_1				=> "{1} looks much more aggressive!",
	  :battler_1			=> :Self,
	  :blankspeech			=> "{2}: (That poor Puppet... Team Rocket really is cruel!)"								
	},
	
	#--------------------------------------------
	# Tenshi is sent out
	# Have Tenshi use Earthquake immediately upon being sent out
	#--------------------------------------------
	"switchInTENSHI_foe" => {
	  :speech				=> "Tenshi, come out and bring down the ceiling!"
	},
	
	"switchSentOutTENSHI_foe" => {
	  :text					=> "{1} used Earthquake to target the ceiling!",
	  :anim					=> :EARTHQUAKE,
	  :wait					=> 20,
	  :playSE  				=> "Mining collapse",
	  :wait_1				=> 20,
	  :battler				=> :Opposing,
	  :text_1    			=> "{1} was struck on the head by a falling rock!",
      :anim    				=> [:ROCKSMASH, :Self],
	  :wait_2				=> 10,
      :hp      				=> -2,
	  :blankspeech			=> ["{2}: Hey! You can't just do that!",
								"{2}: Let's pay them back, {1}!"],
	},

	#--------------------------------------------
	# Sariel is sent out
	# "Not even the domain of Gods will be safe from Team Rocket's influence!", or something like that.
	#--------------------------------------------
	"switchSentOutSARIEL_foe" => {
	  :ignore				=> "afterLastSARIEL_foe",
	  :speech				=> "Behold, as not even the heavens will be safe from Team Rocket's influence!"
	},
	
	"switchSentOutLastSARIEL_foe" => {
	  :ignore				=> "afterNextSARIEL_foe",
	  :speech				=> "Behold, as not even the heavens will be safe from Team Rocket's influence!"
	},
	
	#--------------------------------------------
	# Turns off Switch 123 if you lose
	#--------------------------------------------
	"loss" => {
	  :switchoff			=> 123,
	}
  ...
  #-----------------------------------------------------------------------------
  # Scene: Vs. Test - Purely for testing certain interactions
  #-----------------------------------------------------------------------------  
  VS_TEST = {
	"turnCommand" => {
	  :text      => "Hi!"
  ...
  VS_MISSINGNO = {
	...
	#---------------------------------------------------------------------------
	"turnCommand" => {
	  :text      => [:Opposing, "{1} emited a powerful electro-magnetic pulse!"],
	  :battler	 => :Opposing,
	  :anim		 => :DISCHARGE,
	  :battler_1 => :Self,
	  :anim_1    => :CHARGE,
	...
	#---------------------------------------------------------------------------
    # Continuous - Applies Endure effect to MissingNo whenever targeted by
    #              a damage-dealing move. Ensures it is not KO'd early.
    #              will become disabled once a certain threshold is met.
    #              * Ends after 3rd time getting up.
	#---------------------------------------------------------------------------
	"moveDamaging_repeat" => {
	  :ignore 		=> "variable_3",
	  :battler 		=> :Opposing,
	...
	#---------------------------------------------------------------------------
    # Continuous - MissingNo hits low HP (each time):
    #   * Variable increased by 1.
    #   * Altered stats are reset.
    #   * HP fully restored.
    #   * Ends after 3rd time getting up.
	#---------------------------------------------------------------------------	
    "defenderHPLow_foe_repeat" => {
      :ignore => "variable_3",
	  :setvar => 1,
      :stats  => :Reset,
      :hp     => 1,
	...
	#	* Display a message from the player saying they don't feel so good.
	#	* Display a battle message saying "Your perception of reality is
	#	  becoming warped"
	#	* Lower the active battler's accuracy by 1
	#	* Change the battle background to "Factory Distorted"
	#	* Display: "@ # $ $  & $'  # ^ $ ^ $ $ ; ; ;"
	#---------------------------------------------------------------------------
	"turnCommand_3" => {
	  :backdropnf		=> "factory_distorted",
	  :speech			=> "Is it just me or is everything going out of focus...?",
	  :text				=> "Suddenly, reality around you is becoming warped.",
	  :stats			=> [:ACCURACY, -1],
	  :speech_1			=> [:Opposing, "@ # $ $  & $'  # ^ $ ^ $ $ ; ; ;"]
	},
	...
	#	* Display a message from the Yumemi asking if the player is okay.
	#	* Display a battle message saying "You swear you're seeing numbers all
	#	  around you."
	#	* Lower the active battler's accuracy by 1
	#	* Change the battle background to "Factory Glitch 1"
	#	* Display: "S e r r  z r,  c y r n f r . . ."
	#---------------------------------------------------------------------------
	"turnCommand_6" => {
	  :text				=> "You blink to see if your eyes are playing tricks on you, and...",
	  :backdrop			=> "factory_glitch_1",
	  :bgmnw			=> "W-031. Fires of Hokkai (Pitch Alter 1)",
	  :speech			=> [:Speaker, {:name => "Yumemi", :skin => 1}, "Hey, \\pn, are you okay? You don't look so well."],
	  :text_1			=> "You are starting to see numbers all around you.",
	  :stats			=> [:ACCURACY, -1],
	  :speech_1			=> [:Opposing, "S e r r  z r,  c y r n f r . . ."]
	},
	
	#---------------------------------------------------------------------------
	# Turn 9 - Stage 3 Insanity
	#	* Display a message from Yumemi in binary.
	#	* Display a battle message saying "The world is made of code. All life 
	#	  is quantifiable by ones and zeroes."
	#	* Lower the active battker's accuracy by 1
	#	* Change the battle background to "Factory Glitch 2"
	#	* Display: "F r e e  m e,  p l e a s e . . ."
	#---------------------------------------------------------------------------
	"turnCommand_9" => {
	  :nobgm			=> true,
	  :text				=> "You blink again, and...",
	  :backdrop			=> "factory_glitch_2",
	  :bgmnw			=> "W-031. Fires of Hokkai (Pitch Alter 2)",
	  :text_1			=> "The world is made of code. All life is quantifiable by ones and zeroes.",
	  :speech			=> [:Speaker, {:name => "Yumemi", :skin => 1}, "01001000 01100101 01111001 00100001 00100000 01010011 01101110 01100001 01110000 00100000 01101111 01110101 01110100 00100000 01101111 01100110 00100000 01101001 01110100 00100001"],
	  :stats			=> [:ACCURACY, -1],
	  :speech_1			=> [:Opposing, "F r e e  m e,  p l e a s e . . ."]
	},
	
	#---------------------------------------------------------------------------
	# Turn 18 - Revert to normal
	#	* Display a battle message saying "Reality returns to normal...
	#	  You think?"
	#	* Display a message from the player in parentheticals saying something
	#	  like "(Was that all real?)"
	#	* Change the battle background to "Factory"
	#---------------------------------------------------------------------------
	"turnCommand_18" => {
	  :text				=> "You blink once more, and...",
	  :backdrop			=> "factory",
	  :bgmnw			=> "W-031. Fires of Hokkai",
	  :text_1			=> "Reality returns to normal.. You think?",
	  :speech			=> "(Did that all just happen, or did I imagine it?)"
	},
	
	#---------------------------------------------------------------------------
	# MissingNo hits low HP for the first time:
	#	* "MissingNo lets out a loud cry!" (Play its cry here)
	#   * Display glitched text "A r e  y o u  h e r e  t o  f r e e  m e?"
	#	* Reset all of MissingNo's stats to 0
	#	* Return MissingNo's health to max, give it +1 Defense and SpDefense
	#	* Player: "It stood back up! C'mon {1}, we can do this!"
	#---------------------------------------------------------------------------
	"variable_1" => {
	  :battler 			=> 1,
	  :text				=> "{1} lets out a loud cry!",
	  :speech 			=> [:Anim, :GROWL, :Self, "^ # $  ( % ^  & $ # $  % %  @ # $ $  ! $ !"],
	  :stats			=> [:DEFENSE, 1, :SPECIAL_DEFENSE, 1],
	  :speech_1		 	=> [0, "It stood back up! C'mon {1}, we can do this!"]
	},
	#---------------------------------------------------------------------------
	# MissingNo hits low HP for the second time:
	#	* "MissingNo lets out a deafening cry!" (Play its cry here)
	#	* Display glitched text "I  j u s t  w a n t  t o  b e  f r e e  f r o m  m y  p a i n . . ."
	#	* Reset all of MissingNo's stats to 0
	#	* Return MissingNo's health to max, give it +2 Defense and SpDefense
	#	* Player: "It got back up again!?"
	#	* Yumemi: "It must have inherited Mew's propensity for life! That must
	#	           be why it's still alive after all these years!"
	#---------------------------------------------------------------------------
	"variable_2" => {
	  :battler 			=> 1,
	  :text				=> "{1} lets out a deafening cry!",
	  :speech	 		=> [:Anim, :GROWL, :Self, "$  @ ^ $ %  & ^ ! %  % %  ( $  @ # $ $  @ # % !  ! (  # ^ $ ! ; ; ;"],
	  :stats			=> [:DEFENSE, 2, :SPECIAL_DEFENSE, 2],
      :speech_1 		=> [0, "It got back up again!?",
							:Speaker, {:name => "Yumemi",  :skin => 1}, "It must have inherited Mew's propensity for life! That must be why it's still alive after all these years!"]
	},
	#---------------------------------------------------------------------------
	# MissingNo hits low HP for the third time:
	#	* "MissingNo lets out a earth-shatteringly loud howl!" (Play its cry here)
	#	* Display glitched text "P l e a s e !  I  c a n ' t  t a k e  i t  a n y m o r e !"
	#	* Reset all of MissingNo's stats to 0
	#	* Return MissingNo's health to max, give it +3 Defense and SpDefense
	#	* Player: "How many more times can this thing get back up ?!"
	#	* Yumemi: "It must be close to exhausted by this point! Don't let up!"
	#---------------------------------------------------------------------------
	"variable_3" => {
	  :battler 			=> 1,
	  :text				=> "{1} lets out an earth-shatteringly loud howl!",
	  :speech 			=> [:Anim, :GROWL, :Self, "# ^ $ ^ $ $ ?  *  ) ^ ! | %  % ^ @ $  $ %  ^ ! ( ! % # $ ?"],
	  :stats			=> [:DEFENSE, 3, :SPECIAL_DEFENSE, 3],
      :speech_1 		=> [0, "How many more times can it get back up?!",
							:Speaker, {:name => "Yumemi",  :skin => 1}, "It must be close to exhausted by this point! Don't let up!"]
	},	
	#---------------------------------------------------------------------------
	# MissingNo faints (Turn Count is less than 6 but greater than or equal to 3)
	#	* Display: "% & ^ ! @  * % ^ ; ; ;"
	#	Notes: Scrapped, changed
	#---------------------------------------------------------------------------
	
	#---------------------------------------------------------------------------
	# MissingNo faints (Turn Count is less than 9 but greater than or equal to 6)
	#	* Display: "G u n a x  l b h . . ."
	#	Notes: Scrapped, changed
	#---------------------------------------------------------------------------
	
	#---------------------------------------------------------------------------
	# MissingNo faints (Turn Count is less than 12 but greater than or equal to 9)
