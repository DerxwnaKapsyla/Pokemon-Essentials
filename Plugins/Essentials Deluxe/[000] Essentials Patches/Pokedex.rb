  ...
  def pbGetAvailableForms
    ...
    ret.each do |entry|
      ...
      if !entry[0] || entry[0].empty?
        ...
        case entry[1]
        when 0 then entry[0] = species_data.has_flag?("Puppet") ? _INTL("#{prefix}Yin") : _INTL("#{prefix}Male") # Derx: Changed to display Yin alignment
        when 1 then entry[0] = species_data.has_flag?("Puppet") ? _INTL("#{prefix}Yang") : _INTL("#{prefix}Female") # Derx: Changed to display Yang alignment
        else
