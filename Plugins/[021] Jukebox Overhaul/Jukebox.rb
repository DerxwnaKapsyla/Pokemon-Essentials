...
class EnhancedJukebox
  ...
  # Determined by the SHOWIFVARIABLE and SHOWIFSWITCH tags in the file metadata.
  SPOILER_TEXT = "?????"
  ...
  class FileExplorer
      ...
      when :file
        if item[3] # Contains spoilers
          ...
          @sprites["down"].visible = false
		   ...
		   if pbConfirmMessage("Selected song has not yet been played in game. As such, the comments may contain spoilers. Do you still wish to play this song?")
    ...
    def main
      ...
      if Input.triggerex?(Input::PAGEDOWN)
      ...
      if Input.triggerex?(Input::PAGEUP)
        if @sel >= SCROLL_FROM
  ...
  class SongScreen
    ...
    def main
      ...
      if Input.triggerex?(Input::PAGEDOWN) && can_scroll_down?
      ...
      if Input.triggerex?(Input::PAGEUP) && can_scroll_up?
        @sprites["text"].oy = 0
        @sprites["down"].visible = can_scroll_down?
        @sprites["up"].visible = false
      end
