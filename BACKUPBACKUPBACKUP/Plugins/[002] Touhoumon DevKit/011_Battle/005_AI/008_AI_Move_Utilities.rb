class Battle::AI
  ...
  # Derx: Added checks for Scrappy w/ Nether
  #		  Added checks for Strong Winds w/ Aero
  #		  Added checks for Airborne Targets w/ Aero and Earth
  #		  Added checks for Iron Ball w/ Aero and Earth
  #=============================================================================
  ...
  # Derx: Added checks to Airbrone for Earth
  #		  Added checks to Flash Fire for Pyro
  #		  Added checks to Dry Skin, et al, for Hydro
  #		  Added checks to Sap Sipper for Nature
  #		  Added checks to Lightning Rod, et all, for Wind
  #		  Added checks for Play Ghost
  #		  Did not add checks for Nature immunity to Powder Moves. Still on fence
  #		  Added checks to Prankster for Umbral
  #=============================================================================
  def pbCheckMoveImmunity(score, move, user, target, skill)
    ...
    if skill >= PBTrainerAI.mediumSkill
      ...
      return true if move.statusMove? && Settings::MECHANICS_GENERATION >= 7 &&
                     user.hasActiveAbility?(:PRANKSTER) && (target.pbHasType?(:DARK) || target.pbHasType?(:DARK18)) &&
                     target.opposes?(user)
  ...
  def pbRoughDamage(move, user, target, skill, baseDmg)
    ...
    # Charge
    if skill >= PBTrainerAI.mediumSkill &&
       # Derx: Added a check for Charge effect w/ Wind
       user.effects[PBEffects::Charge] > 0 && (type == :ELECTRIC || type == :WIND18)
      multipliers[:base_damage_multiplier] *= 2
    end
    # Mud Sport and Water Sport
    if skill >= PBTrainerAI.mediumSkill
      # Derx: Added a check for Water Sport and Mud Sport w/ Pyro and Wind
    ...
    if skill >= PBTrainerAI.mediumSkill
      ...
      when :Sun, :HarshSun
        case type
        # Derx: Added a check for Harsh Sun w/ Pyro and Hydro
        when :FIRE, :FIRE18
      ...
      when :Rain, :HeavyRain
        case type
        # Derx: Added a check for Heavy Rain w/ Pyro and Hydro
        when :FIRE, :FIRE18
          multipliers[:final_damage_multiplier] /= 2
        when :WATER, :WATER18
          multipliers[:final_damage_multiplier] *= 1.5
      ...
      when :Sandstorm
        # Derx: Added a check for Sandstorm w/ Beast
        if (target.pbHasType?(:ROCK) || target.pbHasType?(:BEAST18)) && move.specialMove?(type) &&
  ...
  def pbCalcAccuracyModifiers(user, target, modifiers, move, type, skill)
    ...
    if skill >= PBTrainerAI.highSkill
      if move.function == "BadPoisonTarget" &&   # Toxic
         Settings::MORE_TYPE_EFFECTS && move.statusMove? && 
         (user.pbHasType?(:POISON) || user.pbHasType?(:MIASMA18))
