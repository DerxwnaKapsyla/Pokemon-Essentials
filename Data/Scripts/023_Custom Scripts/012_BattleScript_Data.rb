...
# ------ Derx: Battle Scripts related to The Festival of Curses are below here.
   ...
   PossessedPuppet_Intro = Proc.new{|battle|
					...
					$game_variables[2] = battle.battlers[1].species
					pbMessage("\\se[]It's the cursed \\v[1] that Miss Kagiyama told me to find!\1")
				  }
   
   PossessedPuppet_Start = Proc.new{|battle|
					battle.scene.appearBar
					pbPlayCry($game_variables[2])
					Kernel.pbMessageWithName($game_variables[1],"!!!")
					pbMessage("The wild \\v[1] gains power from being possessed!")
					battle.scene.disappearBar
					battle.battlers[1].pbRaiseStatStage(PBStats::ATTACK,1,battle.battlers[1])
					battle.battlers[1].pbRaiseStatStage(PBStats::SPATK,1,battle.battlers[1])
				  }
   ...
   # --- Scene: Vs. Three Fairies of Light
   ThreeFairies_Intro = Proc.new{|battle|
					battle.scene.appearBar
					Kernel.pbMessageWithName($Trainer.name,"Wait! Doesn't three against one seem kinda unfair!")
					Kernel.pbMessageWithName("Star Sapphire","Nuh uh! It's totally fair!")
					Kernel.pbMessageWithName("Luna Child","You've beat us individually! That's only one third of our total strength!")
					Kernel.pbMessageWithName("Sunny Milk","This time we'll fight you all together, one on one!")
					Kernel.pbMessageWithName($Trainer.name,"That logic makes no sense! But fine, we'll play your rules! And when I win, you'll tell me everything you know!")
					battle.scene.disappearBar
				  }
