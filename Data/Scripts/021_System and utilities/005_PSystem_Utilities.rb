  ...
  if event.is_a?(Array)
    ...
    for i in event
      if !done.include?(i.id)
        sprite = $scene.spriteset.addUserAnimation(id,i.x,i.y-1,tinting,2) # Derx: Fixed an "issue" with the exclaim bubble showing up inside overworlds.
        done.push(i.id)
      end
    end
  else
    sprite = $scene.spriteset.addUserAnimation(id,event.x,event.y-1,tinting,2) # Derx: Fixed an "issue" with the exclaim bubble showing up inside overworlds.
  end
