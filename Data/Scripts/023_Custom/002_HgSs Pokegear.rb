  ...
  def start
    ...
    loop do
      ...
      when 5
        ...
        width = setPositionTime[2]; height = setPositionTime[3]
        drawTime(x,y,width,height)
        # Input
        chooseFeature
        # Mouse
        chooseFeatureMouse
...
#-------------------------------------------------------------------------------
  ...
  def setPositionTime
    width = 270; height = 45
    case @background
    when 0; x = (SCREEN_WIDTH-width)/2; y = 59
    when 1; x = (SCREEN_WIDTH-width)/2; y = 59
    when 2; x = 130; y = 78
    when 3; x = 125; y = 60
    when 4; x = (SCREEN_WIDTH-width)/2; y = 50
    else; x = 140; y = 60
    end
    return ret = [x,y,width,height]
  end
  
  def drawTime(x,y,w,h)
    ...
    drawTxt("text",x,y,w,h,string,"Arial")
  end
#-------------------------------------------------------------------------------
  # Customize Page
  def sceneCustomize
...
#-------------------------------------------------------------------------------
  ...
  def nameMusic(list)
    ...
    if list==0
      (0...ListMusicRadio.length).each { |i|
      x = 45
      y = (@select>=max-6 && max>=6)? (5+50*i-(max-6)*50) : (5+50*i-@select*50)
      width = 412; height = 40
      string = ListMusicRadio[i][0]
      drawTxt("text 2",x,y,width,height,string,"Arial",26,0,true) }
    else
      (0...@listmusic.length).each { |i|
      x = 45
      y = (@select>=max-6 && max>=6)? (5+50*i-(max-6)*50) : (5+50*i-@select*50)
      width = 412; height = 40
      string = @listmusic[i]
...
#-------------------------------------------------------------------------------
  ...
  def scenePhone
    ...
    putTrainerList
    @process = 9 if @trainers.length==0
    # Text
    namePhone
    # Arrow
    create_sprite("arrow","Arrow",@viewport)
  ...
  def namePhone(order=nil)
    ...
    (0...max).each { |i|
    x = 20
    y = (num>=max-6 && max>=6)? (5+50*i-(max-6)*50) : (5+50*i-num*50)
    width = 160; height = 40
    if @trainers[i].length==4 
      string = pbGetMessageFromHash(MessageTypes::TrainerNames,@trainers[i][1])
    else
      string = @trainers[i][1]
    end
    drawTxt("text 2",x,y,width,height,string,"Arial",26,0,true)
    if @trainers[i].length==4
      x += width
      width = 252; height = 40
      string = PBTrainers.getName(@trainers[i][0])
      drawTxt("text 3",x,y,width,height,string,"Arial",26,0,true)
    end }
  end
  ...
  def showInformationTrainer(show)
    ...
    if show
      ...
      width = 220; height = 30
      string = "Location :"
      drawTxt("text 4",x,y,width,height,string,"Arial",26,0,true)
      string = (@trainers[pos][2]) ? pbGetMessage(MessageTypes::MapNames,@trainers[pos][2]) : ""
      drawTxt("text 4",x+10,y+35,width,height,string,"Arial",26,0,true)
      # Down
      x = 10; y = @sprites["phone information"].y+114
      width = 330; height = 50
      string = "Registered : #{@trainers.length}"
      drawTxt("text 4",x,y+5,width,height,string,"Arial",26,0,true)
      string = "Waiting for a rematch : #{rematchCount}"
      drawTxt("text 4",x,y+44,width,height,string,"Arial",26,0,true)
    else
  ...
  def choosePhone
    ...
    # Mode: sort
      ...
      if Input.trigger?(Input::UP)
        pbPlayDecisionSE
        @sortselect -= 1
        @sortselect = 0 if @sortselect<0
        namePhone(0) if max>=6
      end
      if Input.trigger?(Input::DOWN)
        pbPlayDecisionSE
        @sortselect += 1
        @sortselect = max-1 if @sortselect>=max
        namePhone(0) if max>=6
      end
      if Input.trigger?(Input::C)
        pbPlayDecisionSE
...
#-------------------------------------------------------------------------------
  ...
  def drawTxt(bitmap,x,y,width,height,string,font=nil,fontsize=26,align=0,nclear=false,bcolor=BaseColor,scolor=ShadowColor)
    # Sprite
