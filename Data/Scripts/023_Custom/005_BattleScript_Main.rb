...
class PokeBattle_DamageState
  ...
  # Derx's changes for the Bahamut fight
  attr_accessor	:veryLowHP
  attr_accessor	:quarteredHP
...
# HP Reduction Dialogue
  def pbReduceHP(amt,anim=true,registerDamage=true,anyAnim=true)
	...
	# --- Derx's changes for the Bahamut Fight ---
    if self.hp < (self.totalhp*0.1).floor && !self.damageState.veryLowHP && self.hp>0
	  self.damageState.veryLowHP = true
	  self.damageState.lowHP = true
	  self.damageState.halfHP = true
	  if !opposes?
		TrainerDialogue.display("veryLowHP",@battle,@battle.scene)
      else
        TrainerDialogue.display("veryLowHPOpp",@battle,@battle.scene)
      end
	elsif self.hp < (self.totalhp*0.25).floor && self.hp > (self.totalhp*0.1).floor && !self.damageState.lowHP 
      self.damageState.lowHP = true
      self.damageState.halfHP = true
      if !opposes?
	...
	elsif self.hp < (self.totalhp*0.75).floor && self.hp > (self.totalhp*0.52) && !self.damageState.quarteredHP
	  self.damageState.quarteredHP = true
	  if !opposes?
		TrainerDialogue.display("quarteredHP",@battle,@battle.scene)
	  else
		TrainerDialogue.display("quarteredHPOpp",@battle,@battle.scene)
	  end
    end
    return amt
  end
...
# Big,Small and Low, Mid HP Dialogue Dialogue
  def pbEndureKOMessage(target)
	...
	if target.hp < (target.totalhp*0.1).floor && target.hp>0
      if !target.damageState.veryLowHP
        target.damageState.lowHP = true
        target.damageState.halfHP = true
		target.damageState.veryLowHP = true
        if !target.opposes?
          TrainerDialogue.display("veryLowHP",@battle,@battle.scene)
        else
          TrainerDialogue.display("veryLowHPOpp",@battle,@battle.scene)
    ...
    elsif target.hp < (target.totalhp*0.25).floor && target.hp>0
      ...
      if !target.damageState.quarteredHP
        target.damageState.quarteredHP = true
        if !target.opposes?
          TrainerDialogue.display("quarteredHP",@battle,@battle.scene)
        else
          TrainerDialogue.display("quarteredHPOpp",@battle,@battle.scene)
...
# Sendout Dialogue
  def pbSendOutBattlers(sendOuts,startBattle=false)
    ...
    sendOuts.each do |b|
      ...
      side=["","Opp"]
      if len2>1
        TrainerDialogue.forceSet("veryLowHP#{side[b[0]]},#{len2-1}")
		TrainerDialogue.forceSet("lowHP#{side[b[0]]},#{len2-1}")
        TrainerDialogue.forceSet("halfHP#{side[b[0]]},#{len2-1}")
		TrainerDialogue.forceSet("quarteredHP#{side[b[0]]},#{len2-1}")
