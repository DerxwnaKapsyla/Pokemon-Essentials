...
class CoinMart_Scene
  ...
  def pbRefresh
    ...
    @sprites["moneywindow"].text=_INTL("Festival Points:\r\n<r>{1}",@adapter.getPointsString)
  end
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if pbIsImportantItem?(item)
        if !pbConfirm(_INTL("Certainly. You want {1}. That will be {2} FP. OK?",
           itemname,price.to_s_formatted))
          next
        end
        quantity=1
      else
        maxafford=(price<=0) ? BAG_MAX_PER_SLOT : @adapter.getPoints/price
        ...
        next if quantity==0
        price*=quantity
        if !pbConfirm(_INTL("{1}, and you want {2}. That will be {3} FP. OK?",
  ...
  def command_302
    ...
    loop do
      ...
      if @list[@index].code == 605
        ...
        # End
        pbCoinMart(shop_goods.compact,nil,@params[2])
        return true
      end
    end
  end
